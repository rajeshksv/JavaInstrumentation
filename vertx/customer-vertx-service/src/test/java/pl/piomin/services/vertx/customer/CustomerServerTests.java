...
public class CustomerServerTests {
    ...
    @Test
    @Order(3)
    void shouldFindByIdWithAccounts(Vertx vertx, VertxTestContext testContext) {
        WebClient client = WebClient.create(vertx);
        client.get(3333, "localhost", "/customer/" + id + "/with-accounts")
                .send()
                .onSuccess(res -> {
                    LOGGER.info("Customer with accounts: " + res.bodyAsString());
                    assertNotNull(res.body());
                    Customer customer = res.bodyAsJson(Customer.class);
                    assertNotNull(customer.getId());
                    // Note: accounts might be empty if no account service is running
                    // This test verifies the endpoint works and returns the customer
                    testContext.completeNow();
                })
                .onFailure(testContext::failNow);
    }
