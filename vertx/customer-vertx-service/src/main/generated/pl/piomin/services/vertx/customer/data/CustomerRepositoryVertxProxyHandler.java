...
public class CustomerRepositoryVertxProxyHandler extends ProxyHandler {
  ...
  public void handle(Message<JsonObject> msg) {
    try{
      ...
      switch (action) {
        case "save": {
          service.save(json.getJsonObject("customer") != null ? new pl.piomin.services.vertx.customer.data.Customer((JsonObject)json.getJsonObject("customer")) : null).onComplete(res -> {
                        if (res.failed()) {
        ...
        case "findAll": {
          service.findAll().onComplete(res -> {
                        if (res.failed()) {
                          HelperUtils.manageFailure(msg, res.cause(), includeDebugInfo);
                        } else {
        ...
        case "findById": {
          service.findById((java.lang.String)json.getValue("id")).onComplete(res -> {
                        if (res.failed()) {
                          HelperUtils.manageFailure(msg, res.cause(), includeDebugInfo);
                        } else {
                          msg.reply(res.result() != null ? res.result().toJson() : null);
        ...
        case "findByName": {
          service.findByName((java.lang.String)json.getValue("name")).onComplete(res -> {
                        if (res.failed()) {
                          HelperUtils.manageFailure(msg, res.cause(), includeDebugInfo);
                        } else {
                          msg.reply(new JsonArray(res.result().stream().map(v -> v != null ? v.toJson() : null).collect(Collectors.toList())));
        ...
        case "remove": {
          service.remove((java.lang.String)json.getValue("id")).onComplete(HelperUtils.createHandler(msg, includeDebugInfo));
          break;
        }
